...
$\zeta(\symbf{s}_0)=\frac{1}{\mbox{Var}(\hat{S}(\symbf{s}_0)-S(\symbf{s}_0))}$. The higher $\zeta(\symbf{s}_0)$ is, the better quality the prediction has, and we give more weight to the most reliable pollution values while aggregating them [@SanchezMeca1998; @LeeC2016]. 

After obtaining pollution predictions at the center of all grids where the population density data are available (see Figure \ref{fig:maps2}) using (\ref{eq:prediction}), and denoting the population density at location $\symbf{s}_i$ as $G(\symbf{s}_i)$, for a specific county $k$, the spatially representative pollution concentration is estimated by 
\begin{equation}
...
Recall that the disease data are counts of the numbers of cases occurring in each county in Germany, and that the observed and expected number of COVID-19 cases for county $k$ are denoted as $Y_{k}$ and $E_{k}$, respectively.  The health model is a Poisson log-linear model [see @Shaddick2015], given by 
